...
import type { PnlPeriod, PnlSummaryItem, PnlDetailAccountItem } from '@domain/entities/PnlReport'
import { usePnlDetail } from '@application/hooks/report/usePnlDetail'
        ...
        <div key={`${item.code}-${idx}`}>
          <div
            className={cn(
              ...
              variant === 'card'
                ? cn('px-3 py-2 text-[12px] leading-5', depth === 0 && 'bg-surface-muted/50 font-bold')
                : cn('px-4 py-2 text-[12.5px] leading-5', depth === 0 && 'bg-[#F9F9FB] font-bold'),
            )}
            style={{ paddingLeft: `${(variant === 'card' ? 12 : 16) + depth * 16}px` }}
          >
            <span className={variant === 'card' ? 'text-text' : 'text-[#1C1C1E]'}>{item.name}</span>
            <span
              className={cn(
                'text-right tabular-nums pl-3 shrink-0',
                variant === 'card' ? 'text-text' : 'text-[#1C1C1E]',
              )}
            >
              {nominal(item.valueCents)}
            </span>
          </div>
          {item.children.length > 0 ? (
            <div className={variant === 'card' ? 'divide-y divide-border/30' : 'divide-y divide-[#F2F2F4]'}>
              <PnlAccountRows items={item.children} depth={depth + 1} variant={variant} />
            </div>
          ) : null}
        </div>
      ))}
...
function PnlDetailTable({ items }: { items: PnlDetailAccountItem[] }): ReactNode {
  return (
    <div className="rounded-2xl overflow-hidden border border-border">
      ...
      <div className="divide-y divide-border/60 bg-surface-raised">
        <PnlAccountRows items={items} depth={0} variant="card" />
      </div>
    </div>
  )
}
...
export function MonthlyStatementCard({
  ...
  const { detail, isLoading: isDetailLoading, fetchDetail } = usePnlDetail()
  const pdfContentRef = useRef<HTMLDivElement | null>(null)

  const netProfitCents = Math.abs(item.netProfitCents)

  const ensureDetail = async () => detail ?? (await fetchDetail(period, item.sectionKey))

  const handleOpenDetail = () => {
    setShowDetail(true)
    if (!detail) void fetchDetail(period, item.sectionKey)
  }
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const resolvedDetail = await ensureDetail()
      ...
      const params: GenerateMonthlyPdfParams = {
        ...
        cooperativeUnitName,
        operatingRevenueCents: item.omzetCents,
        operatingExpensesCents: item.expenseCents,
        netProfitCents: Math.abs(resolvedDetail.netProfitCents),
      }
      ...
      <Card padding="md" className={cn(isFirst ? '' : '')}>
        <div className="h-stack justify-between mb-3">
          <div className="h-stack gap-2">
            ...
            <div>
              <h4 className="text-sm font-bold leading-5 text-text">{item.periodLabel}</h4>
        ...
        <div className="grid grid-cols-2 gap-2.5 mt-4 mb-3">
          <div className="col-span-2 rounded-xl bg-surface-muted p-3 space-y-1">
            ...
            <RupiahText cents={item.omzetCents} size="sm" />
          </div>
          <div className="col-span-2 rounded-xl bg-surface-muted p-3 space-y-1">
            <div className="h-stack gap-1 text-[11px] text-text-muted">
              <ArrowDownRight className="h-3.5 w-3.5 text-danger-text" />
              {USER_STRINGS.dashboard.shuAllocationOperatingExpense}
            </div>
            <RupiahText cents={item.expenseCents} size="sm" />
          </div>
          <div className="col-span-2 rounded-xl bg-surface-muted p-3 space-y-1">
            <div className="h-stack gap-1 text-[11px] text-text-muted">
            ...
            <RupiahText cents={netProfitCents} size="sm" tone="success" showZeroAsDash={false} />
          </div>
        </div>
          ...
          <div className="w-[780px] bg-white p-6 font-sans">
            <div className="mb-4">
              ...
              <div className="text-[13px] text-[#525258]">
                Periode: <span className="font-semibold">{item.periodLabel}</span>
                {cooperativeUnitName ? (
                  <>
            ...
            <div className="rounded-2xl overflow-hidden border border-[#E8E8EC]">
              ...
              <div className="divide-y divide-[#E8E8EC] bg-white">
                <PnlAccountRows items={detail?.items ?? []} depth={0} variant="pdf" />
              </div>
            </div>
            <div className="mt-6">
              <div className="grid grid-cols-[1fr_auto] px-4 py-3 rounded-xl bg-[#FEF1F3] border border-[#FCC7D2] text-[15px] font-bold items-center">
                ...
                <span className="tabular-nums text-right pl-3 shrink-0 text-[#059669]">
                  {nominal(Math.abs(detail?.netProfitCents ?? 0))}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="px-5 pt-5 pb-3 flex items-start justify-between gap-3 border-b border-border/60">
              <div className="h-stack gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-sm leading-5 text-text-body">{item.periodLabel}</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="px-5 py-4 overflow-y-auto">
              {isDetailLoading && !detail ? (
                <div className="space-y-2">
                  <Skeleton widthClass="w-full h-10 rounded-xl" />
                  <Skeleton widthClass="w-full h-10 rounded-xl" />
                  <Skeleton widthClass="w-full h-10 rounded-xl" />
                </div>
              ) : (
                <>
                  <PnlDetailTable items={detail?.items ?? []} />
                  <div className="mt-4">
                    <div className="grid grid-cols-[1fr_auto] px-3 py-2.5 rounded-xl bg-brand-50 border border-brand-100 text-sm font-bold items-center">
                      ...
                      <span className="tabular-nums text-right pl-3 shrink-0 text-success-text">
                        {nominal(Math.abs(detail?.netProfitCents ?? 0))}
                      </span>
                    </div>
                  </div>
