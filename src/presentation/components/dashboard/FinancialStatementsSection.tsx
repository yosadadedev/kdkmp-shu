...
import { usePnlSummary } from '@application/hooks/report/usePnlSummary'
...
export function FinancialStatementsSection() {
  const [activeTab, setActiveTab] = useState<PnlPeriod>('monthly')
  const { items, isLoading } = usePnlSummary(activeTab)
        ...
        {isLoading ? (
          ...
          items.map((item, idx) => (
            <MonthlyStatementCard
              ...
              period={activeTab}
              item={item}
              cooperativeUnitName={DEFAULT_COOPERATIVE_UNIT.branchName}
              isFirst={idx === 0}
            />
          ))
        )}
