...
import { useShuReportCard } from '@application/hooks/report/useShuReportCard'
import { cn } from '@presentation/utils/cn'
import { Skeleton } from '@presentation/components/ui/Skeleton'

export function ProfitSharingAmountCard() {
  const { reportCard, isLoading } = useShuReportCard()
    ...
    >
      ...
      <div className="relative h-stack justify-between pt-3 border-t border-white/15">
        <div className="h-stack gap-2 text-[14px] text-white/95">
          ...
          <span className="font-bold">{reportCard.period}</span>
        </div>
      </div>
    </Card>
  )
