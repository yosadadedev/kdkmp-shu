...
import { hashNationalIdDummy, normalizeNationalId } from '@domain/value-objects/NationalId'
...
const buildAuthSessionForMember = (memberId: string): AuthenticatedSession => {
  ...
  return {
    ...
    // Overwritten by HttpAuthRepository with the real backend values right
    // after this mock session is built — this repository is never used
    // standalone, so these never reach anywhere that reads them.
    userNik: '',
    companyNik: '',
  }
}
...
export class MockAuthRepository implements AuthRepository {
  async sendOtpByNationalId(nationalIdNikPlain: string): Promise<SendOtpResult> {
    await mockDelayMs()
    try {
      // Format/registration is already validated by the real backend
      // (validateNik) before this fallback runs — don't re-gate on the
      // 3-member mock seed here, or a real, already-confirmed NIK that
      // isn't one of those 3 would get wrongly rejected.
      const normalized = normalizeNationalId(nationalIdNikPlain)
      const hash = hashNationalIdDummy(normalized)
      const member = findMemberByNikPlain(normalized)
      const memberId = member?.id ?? `mb-${hash.slice(0, 12)}`
      const maskDestination = member?.phoneNumberMasked ?? '+62 8**-****-****'
      const sessionId = genSessionId()
      const session = buildOtpSession(sessionId, memberId, hash, maskDestination)
